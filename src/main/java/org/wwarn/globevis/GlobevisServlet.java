...
//import org.wwarn.globevis.poem.GlobevisDatabase;

/**
 * Base servlet for Globevis servlets.
 */
public abstract class GlobevisServlet extends TemplateServlet {


  public static final String templatePrefix = "org/wwarn/globevis/view/";
  ...
  private boolean fileAt(String filename){
    ...
    if (filename.equals("/")) return false;
    String fsName = "/dist/globevis/www" + filename;
    File it = new File(fsName);
    System.err.println("FS:" + fsName + " " + it.exists());
    return it.exists();
  }

  public String globevisTemplate(String name) {
  ...
  protected String doTemplateRequest(Melati melati, ServletTemplateContext context)
      throws Exception {
    return globevisTemplate(reallyDoTemplateRequest(melati, context));
  }
